...
| A13 | **Avisar a gestão pelo WhatsApp** quando houver pedido de cancelamento | ⚪ | 💻 | **Adiado em 22/09** junto com o WhatsApp Business: depende do número dedicado e da conta na Meta (docs/06-WHATSAPP.md §3), e o e-mail + o alerta no painel já cobrem o aviso. Decidir junto com o projeto de WhatsApp (lembretes, lista de espera), não sozinho. Encaixa no `solicitar_cancelamento_plano()` quando existir a Edge Function `whatsapp-enviar` (§12). |
...
| A15 | ~~Acúmulo de créditos do semestral não acontece~~ (regulamento 3.4) | ✅ | 💻 | **Corrigido em 22/09** (`20260922120000`, testado no DEV com rollback). Na renovação, o saldo DO PLANO que sobra passa para o ciclo seguinte até `teto_acumulo_ciclos` × créditos (1 ciclo: o saldo nunca passa do dobro) — o lote continua o mesmo, com a validade do ciclo novo; o excedente sai dos lotes mais antigos. No fim do compromisso tudo expira (3.6). **Cortesia e reposição não entram no limite nem expiram na renovação** (decisão da gestão em 22/09) — antes, a renovação do mensal as matava junto com o ciclo. Quem paga no dia do vencimento (já marcado inadimplente de madrugada) não perde o acúmulo: `processar_assinaturas()` segura a expiração do saldo do semestral até a renovação. |
...
| A17 | ~~Ciclo tem 31 dias, não 30~~ (regulamento 3.1) | ✅ | 💻 | **Resolvido em 22/09** (`20260922120000`): a gestão escolheu o **mês civil, sempre no mesmo dia** — é como o Asaas cobra (a assinatura dele não tem "a cada 30 dias"; dia 29–31 cai no último dia de mês curto e volta no seguinte). `produtos.periodicidade_meses` (assinatura sempre tem, por constraint), `matriculas.dia_renovacao` e uma conta só, `data_renovacao()`. Compra única segue em dias. ⚠️ **O texto do regulamento muda junto** (3.1, resumo, formatos Mensal e Turma Fixa) — pendente com a gestão, e as cópias em `docs/marketing/07`, `08` e `09` também. |
