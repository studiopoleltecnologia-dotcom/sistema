...
const PASTA = { dia: 'A-stories-por-dia', m1: 'M1-vao-esquerda', m2: 'M2-etiqueta',
  c1: 'C1-coluna-fixa', c2: 'C2-coluna-sob-demanda' }
...
const pranchas = await p.evaluate(() =>
  [...document.querySelectorAll('.prancha')].map(el => ({
    id: el.id,
    grupo: el.classList.contains('dia') ? 'dia' : ['m1','m2','c1','c2'].find(c => el.classList.contains(c)),
  })))

for (const { id, grupo } of pranchas) {
  const chave = id.replace(/^(dia|m1|m2|c1|c2)-/, '')
...
console.log('PDF de 18 páginas →', `${DEST}/grade-2-salas.pdf`)
